...
for file in $mc_dir/* ; do
    ...
    if [ ! -d $out_dir/firstwarps ] ; then
        mkdir -p $out_dir/firstwarps
    fi
    ...
    if [ ! -d $out_dir/combinedwarps ] ; then
        mkdir -p $out_dir/combinedwarps
    fi
    ...
    if [ ! -d $out_dir/topupwarps ] ; then
        mkdir -p $out_dir/topupwarps
    fi

    convertwarp --warp1=$firstwarp --warp2=$intermediatewarp --out=$topupwarp --ref=$epi_vol --rel --relout -v

    # Apply warps to MNI
    if [ ! -d $out_dir/warped_mni ] ; then
        mkdir -p $out_dir/warped_mni
    fi
    ...
    if [ ! -d $out_dir/warped_topup ] ; then
        mkdir -p $out_dir/warped_topup
    fi    
...
fslmerge -tr $warped_mni `ls $out_dir/warped_mni/epi_rest_*.nii.gz` 2.0
fslmerge -tr $out_dir/epi_rest_topup.nii.gz `ls $out_dir/warped_topup/epi_rest_*.nii.gz` 2.0
