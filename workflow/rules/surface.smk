...
rule apply_surf_tkr2scanner:
    ...
    output: 'output/midthickness_surface_anat/{subject}/{hemi}.midthickness.surf.gii'
    threads: 8
    container: config['connectome_workbench']
    shell: 
...
rule inflate_surface:
    ...
    output:
        inflated = 'output/midthickness_surface_anat/{subject}/{hemi}.inflated.surf.gii',
        very_inflated = 'output/midthickness_surface_anat/{subject}/{hemi}.very_inflated.surf.gii',
...
rule apply_surf_scanner2epi:
    ...
    output: 'output/midthickness_surface_epi/{subject}/{hemi}.midthickness.surf.gii'
    threads: 8
    container: config['connectome_workbench']
    shell:     
...
rule resample_surface:
    input: 
        ...
        sphere_new = 'cfg/fs_LR-deformed_to-fsaverage.{hemi}.sphere.32k_fs_LR.surf.gii'
    output: 'output/midthickness_surface/{subject}/{hemi}.midthickness.32k_fs_LR.surf.gii'
    container: config['connectome_workbench']      
    shell:    
...
rule resample_surface_data:
    ...
    output: 'output/epi_surf_32k_fs_LR/{subject}/{hemi}.rsfMRI.32k_fs_LR.func.gii'
    threads: 8
    container: config['connectome_workbench']    
    shell:     
